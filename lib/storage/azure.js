...
module.exports = function() {
  ...
  var self = {
    ...
    copyOut: function(path, localPath, options, callback) {
      console.log("az copyOut", arguments)
      var blob = blobSvcs[0]
      path = path[0] === '/' ? path.slice(1) : path;
    ...
    disable: function(path, callback) {
      const blob = blobSvcs[0];
      const dPath = utils.getDisabledPath(path);
      blob.startCopyBlob(path, blob.container, dpath, {}, callback);
    ...
    enable: function(path, callback) {
      const blob = blobSvcs[0];
      const dPath = utils.getDisabledPath(path);
      blob.startCopyBlob(dpath, blob.container, path, {}, callback);
      return callback(null);
    },
